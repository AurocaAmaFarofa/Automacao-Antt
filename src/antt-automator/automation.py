...
async def consultar_placa(pagina, dados_placa, pasta_destino):
    ...
    if rntrc_normalizado:
        ...
        tipo_documento = (
            "CNPJ" if len(cpf_cnpj_normalizado) > 11 else "CPF"
        )
    ...
    print(
        f"Campos preenchidos. Documento utilizado: "
        f"{tipo_documento or 'nenhum'}"
    )
    ...
    try:
        ...
        if (
            pdf_bytes_capturado is None
            and "AbrePDF.aspx" in nova_pagina.url
        ):
            try:
                resposta_direta = await pagina.context.request.get(
                    nova_pagina.url
                )
    ...
    if pdf_bytes_capturado is not None:
        ...
        print("\n================================")
        print(
            ">>> PDF NÃO FOI CAPTURADO "
            "(resposta pode ter sido HTML/expirada)"
        )
        print("================================")
